...
function useGeolocation() {
  ...
  const setManualLocation = useCallback((pos, label) => {
    setUserLocation(pos); setLocationLabel(label); setStatus("granted");
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (!ac.signal.aborted && mountedRef.current) setSearchError("Search failed. Try again.");
      } finally {
...
export default function MassageClinic() {
  ...
  useEffect(() => {
    if (userLocation && !searchLocation && viewMode === "nearby") setSearchLocation(userLocation);
  }, [userLocation, searchLocation, viewMode]);

  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === "visible" && needsRefetchRef.current) {
        needsRefetchRef.current = false; loadClinics();
  ...
  const loadClinics = useCallback(async () => {
    ...
    try {
      // ── KEY FIX: select owner_id (schema column) not user_id ──────
      let query = supabase
  ...
  useEffect(() => {
    if (viewMode === "all" || (viewMode === "nearby" && searchLocation)) loadClinics();
  ...
  const handleDeleteRequest = useCallback((clinic) => {
    setDeleteError(""); setDeleteTarget(clinic);
  }, []);
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      // ── KEY FIX: guard with owner_id (schema column) ──────────────
      const { error: delErr } = await supabase
        .from("massage_clinics")
        .delete()
        .eq("id",       deleteTarget.id)
        .eq("owner_id", user.id); // ← correct schema column
      ...
      setDeleteError(err?.message || err?.error || "Failed to delete. Please try again.");
    } finally {
      setIsDeleting(false);
    }
  ...
  const handleLocationSelect = (pos, label) => {
    setSearchLocation(pos); setManualLocation(pos, label);
    setViewMode("nearby"); setShowLocationFilter(false);
  };
  ...
  const toggleViewMode = (mode) => {
    setViewMode(mode);
    if (mode === "all") setSearchLocation(null);
    else if (mode === "nearby" && userLocation) setSearchLocation(userLocation);
  };

  const effectiveLocationLabel =
    searchLocation === userLocation ? locationLabel
    : searchLocation ? "Selected location"
    : locationLabel;
                ...
                <div className="flex bg-gray-100 rounded-full p-1">
                  <button
                    ...
                    className={`py-1.5 px-4 text-xs font-bold rounded-full transition-all ${
                      viewMode === "nearby" ? "bg-white shadow text-violet-700" : "text-gray-500 hover:text-gray-800"
                    }`}
                  >
                    Near Me
                  </button>
                  <button
                    ...
                    className={`py-1.5 px-4 text-xs font-bold rounded-full transition-all ${
                      viewMode === "all" ? "bg-white shadow text-violet-700" : "text-gray-500 hover:text-gray-800"
                    }`}
                  >
                    All
                  </button>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <div className="relative" ref={locationFilterRef}>
                  ...
                  >
                    ...
                    <span className="truncate max-w-[140px]">{effectiveLocationLabel || "Set location"}</span>
                  ...
                  {showLocationFilter && (
                    <div className="absolute left-0 top-full z-50 mt-2 rounded-2xl border border-gray-100 bg-white shadow-2xl min-w-[260px]">
                      <LocationSearchBar onSelect={handleLocationSelect} onUseMyLocation={handleUseMyLocation} />
                    </div>
                  )}
                </div>
...
/* ================================================================
   CLINIC CARD — ownership via owner_id
   ================================================================ */
...
function ClinicCard({ clinic, viewMode, currentUserId, onEdit, onDelete }) {
  ...
  // ── KEY FIX: compare against owner_id (schema column) ────────────
    ...
    >
      ...
      <div className="relative h-48 sm:h-44 lg:h-48 overflow-hidden bg-gradient-to-br from-violet-100 to-fuchsia-100 shrink-0">
        {clinic.cover_url && !imageError ? (
          <img
            src={clinic.cover_url} alt={clinic.name || "Clinic"}
        ...
        {/* Top-right: owner menu OR badges */}
        <div
        ...
        >
          ...
          {!isOwner && clinic.distance_km != null && viewMode === "nearby" && (
        ...
        {/* Owner — show badges below the menu */}
        {isOwner && (
          <div className="absolute top-14 right-3 flex flex-col items-end gap-1.5 z-10 pointer-events-none">
