...
function Toast({ message, type = "success" }) {
  ...
  return (
    <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
      <div className={`flex items-center gap-2.5 rounded-2xl px-5 py-3 shadow-2xl border text-sm font-semibold ${isError ? "bg-red-500 border-red-400 text-white" : "bg-white border-gray-200 text-gray-900"}`}>
        {isError ? <AlertIcon className="w-4 h-4 shrink-0" /> : <CheckIcon className="w-4 h-4 shrink-0 text-green-500" />}
        {message}
      </div>
    </div>
  );
      ...
      <div className="relative z-10 w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl">
        ...
        <div className="mt-6 flex gap-3">
          ...
          <button onClick={onConfirm} disabled={isDeleting}
            ...
            {isDeleting ? <><SpinnerIcon className="w-4 h-4" />Deleting…</> : "Delete"}
          </button>
        </div>
      </div>
    </div>
    ...
    <div ref={menuRef} className="relative" onClick={(e) => e.stopPropagation()}>
      ...
      {open && (
        <div className="absolute right-0 top-10 z-30 min-w-[140px] rounded-2xl bg-white border border-gray-100 shadow-xl py-1 overflow-hidden">
          <button onClick={() => { setOpen(false); onEdit(clinic); }}
          ...
          <button onClick={() => { setOpen(false); onDelete(clinic); }}
  ...
  return (
    <div className="p-3 border-b border-gray-100">
      ...
      {results.map((result, i) => (
        <button key={i}
          onClick={() => onSelect({ lat: result.lat, lng: result.lng }, result.label.split(",")[0].trim())}
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex bg-gray-100 rounded-full p-1">
                  <button onClick={() => toggleViewMode("nearby")}
                    className={`py-1.5 px-4 text-xs font-bold rounded-full transition-all ${viewMode === "nearby" ? "bg-white shadow text-violet-700" : "text-gray-500 hover:text-gray-800"}`}>
                    Near Me
                  </button>
                  <button onClick={() => toggleViewMode("all")}
                    className={`py-1.5 px-4 text-xs font-bold rounded-full transition-all ${viewMode === "all" ? "bg-white shadow text-violet-700" : "text-gray-500 hover:text-gray-800"}`}>
                    All
                  </button>
                </div>
              </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 z-10">
          ...
          {rating > 0 && (
            <div className="flex items-center gap-1">
              ...
              {reviewCount > 0 && <span className="text-[11px] text-white/70">({reviewCount})</span>}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        {clinic.specialties?.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-3">
            ...
            {clinic.specialties.length > 3 && (
              <span className="text-[10px] text-gray-400 px-1 py-0.5">+{clinic.specialties.length - 3}</span>
            )}
          </div>
        )}
        ...
        <div className="flex gap-2 pt-3 border-t border-gray-100">
          ...
          <button
            ...
            className={`flex items-center justify-center gap-1.5 bg-violet-600 hover:bg-violet-700 text-white py-2.5 rounded-xl text-xs font-bold transition-colors shadow-sm ${clinic.phone || isOwner ? "flex-1" : "w-full"}`}>
