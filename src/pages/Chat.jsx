...
export default function Chat(){
  ...
  useEffect(() => {
    chatService.getConversation(id).then(setMsgs).catch(console.error);
  ...
  const send = async () => {
    ...
    try {
      await chatService.sendToConversation({ id, text: t });
    } catch(e) {
      console.error(e);
      setText(t);
    }
  ...
  return (
    <div className="flex min-h-dvh flex-col">
      ...
      <div className="flex-1 space-y-3 bg-gray-50 p-4 overflow-y-auto">
        {msgs.map(m=>(
          <div key={m.id} className={`max-w-[75%] rounded-2xl px-4 py-3 text-sm ${m.sender_id === supabase.auth.getUser()?.id ? "ml-auto bg-violet-600 text-white" : "bg-white shadow-card"}`}>
            {m.text}
          </div>
        ))}
        <div ref={bottomRef} />
      ...
      <div className="sticky bottom-0 flex items-center gap-2 border-t bg-white p-3">
        ...
        <button onClick={send} className="rounded-full bg-violet-600 p-3 text-white" aria-label="Send"><i className="lni lni-telegram text-xl -rotate-12" /></button>
      </div>
    </div>
  );
}