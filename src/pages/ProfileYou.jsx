...
function Toast({ toast }) {
  ...
  return (
    <div className="fixed left-1/2 top-20 z-50 -translate-x-1/2 animate-in
      slide-in-from-top duration-300 pointer-events-none">
...
function HeroSection({
  ...
  return (
    <div className="relative h-[48vh] w-full overflow-hidden
      bg-gradient-to-br from-violet-100 via-fuchsia-50 to-pink-100">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="absolute inset-0 bg-gradient-to-t
            from-black/75 via-black/25 to-black/10" />
        </>
      ) : (
        <div className="absolute inset-0 flex flex-col items-center
          justify-center gap-3 text-violet-400">
          <div className="h-20 w-20 rounded-full bg-violet-100 flex
      ...
      <div className="absolute inset-x-0 top-0 p-4
        ...
        <div className="rounded-full bg-black/40 backdrop-blur-md
          border border-white/20 px-3.5 py-2">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-full
              ...
              hover:bg-black/55 active:scale-95 transition-all"
          >
          ...
          <button
            onClick={onAddPhotos}
            className="inline-flex items-center gap-1.5 rounded-full
              bg-white px-4 py-2 text-xs font-bold text-gray-900
              shadow-lg hover:bg-white/90 active:scale-95 transition-all"
          >
          ...
          {heroUrl && heroPath && (
            <button
              ...
              className="h-9 w-9 flex items-center justify-center rounded-full
                ...
                hover:bg-red-500/80 active:scale-95 transition-all"
...
function PhotoTile({ src, index, isPrimary, onMakePrimary, onDelete }) {
  ...
  if (imgError) {
    return (
      <div className="aspect-square rounded-2xl bg-gray-100 border
        border-dashed border-gray-200 flex flex-col items-center
  ...
  return (
    <div className="group relative aspect-square overflow-hidden rounded-2xl
      ...
      {/* Primary badge */}
      {isPrimary && (
        <div className="absolute left-2 top-2 inline-flex items-center gap-1
          rounded-full bg-violet-600 px-2.5 py-1 text-[10px] font-bold
        ...
        <div className="absolute inset-x-2 bottom-2 flex gap-1.5">
          {!isPrimary && (
            <button
              ...
              className="flex-1 rounded-xl bg-white/95 py-1.5 text-[11px]
                font-bold text-gray-900 hover:bg-white active:scale-95
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-xl
              ...
              transition-all shadow disabled:opacity-50"
          >
            <TrashIcon className="h-4 w-4" />
          </button>
        </div>
...
function SectionCard({ children, className = "" }) {
  return (
    <section className={`rounded-3xl bg-white border border-gray-100
...
export default function ProfileYou() {
  ...
  // gallery (non-primary photos)
  const [gallery,   setGallery]   = useState([]); // { url, path }[]
  ...
  const completeness = useMemo(() => {
    ...
    else hints.push("Add your city");

    if (profession?.trim())   score += 5;
    else hints.push("Add your profession");
  ...
  const handlePhotoUpload = async (files) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const { error: insErr } = await supabase
          .from("photos")
          .insert({ user_id: user.id, path: fileName, is_primary: false });
        if (insErr) throw insErr;
      ...
      if (!heroUrl && uploaded.length > 0) {
        // Make first upload primary
        await makePrimaryPhoto(uploaded[0].path, /* silent */ true);
  ...
  const makePrimaryPhoto = async (path, silent = false) => {
    ...
    try {
      ...
      // Relocate old hero → gallery; remove promoted photo from gallery
      setGallery((prev) => {
  ...
  const deletePhoto = async (path) => {
    ...
    try {
      ...
      if (deletingHero) {
        // Promote first gallery photo as new hero, or clear hero
  ...
  /* ── Render ─────────────────────────────────────────────────── */
  return (
    <div className="min-h-screen bg-gray-50 pb-28 antialiased">
      <TopBar title="" />
      <Toast toast={toast} />
      ...
      <div className="-mt-4 space-y-4 px-4 relative z-10">
            ...
            <div className="h-2 w-full overflow-hidden rounded-full bg-violet-200">
              <div
                className="h-full rounded-full bg-gradient-to-r
                  from-violet-500 to-fuchsia-500 transition-all duration-300"
              ...
              <>
                ...
                <div className="flex gap-2 items-end">
                  ...
                  <button
                    ...
                    className="mb-0.5 flex h-[46px] w-[46px] shrink-0 items-center
                      justify-center rounded-2xl border border-gray-200
                      text-violet-600 hover:bg-violet-50 active:scale-95
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full resize-none rounded-2xl border border-gray-200
                      p-4 text-sm text-gray-900 placeholder:text-gray-400
                      focus:border-violet-400 focus:ring-2 focus:ring-violet-100
                      outline-none transition-all"
                  />
                  <div className="flex items-center justify-between mt-1.5 px-1">
                    <span className={`text-xs ${
                      bio.length >= 50 ? "text-green-600 font-medium" : "text-gray-400"
                    }`}>
                      {bio.length >= 50 ? "✓ Looks great!" : `${50 - bio.length} more characters to go`}
                    </span>
                    <span className={`text-xs font-medium ${
                      bio.length >= MAX_BIO_LENGTH ? "text-red-500" : "text-gray-400"
                    }`}>
            ...
            action={
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-2xl
                  bg-violet-600 px-3.5 py-2 text-xs font-bold text-white
            ...
            {loading ? (
              ...
              <div className="rounded-2xl border-2 border-dashed border-gray-200
                ...
                <button
                  onClick={() => fileInputRef.current?.click()}
                  className="inline-flex items-center gap-1.5 rounded-2xl
                    bg-violet-100 px-5 py-2.5 text-sm font-bold text-violet-700
                ...
                {totalPhotos < MAX_PHOTOS && (
                  <button
                    ...
                    className="aspect-square rounded-2xl border-2 border-dashed
                      ...
                      gap-2 text-gray-400 hover:border-violet-300
          ...
          <div className="p-5 space-y-3">
            <button
              ...
              className="flex w-full items-center justify-center gap-2
                rounded-2xl border border-gray-200 bg-white py-3 text-sm
                font-bold text-gray-800 hover:bg-gray-50 active:scale-[0.98]
                transition-all"
            >
            ...
            <button
              ...
              disabled={loggingOut}
              className="flex w-full items-center justify-center gap-2
                rounded-2xl bg-red-500 py-3 text-sm font-bold text-white
        ...
        {/* ══ Motivational footer ════════════════════════════════ */}
        <div className="rounded-3xl bg-gradient-to-br from-violet-600
          to-fuchsia-600 p-6 text-white shadow-lg shadow-violet-200">
          <div className="flex items-start gap-4">
            <div className="h-12 w-12 rounded-2xl bg-white/20 flex items-center
              justify-center shrink-0 text-2xl">
              ✨
            </div>
            <div>
