      ...
      <Stage>
        ...
        {isPaused && (
          <div className="pointer-events-none absolute inset-0 z-20 grid place-items-center">
            <div className="grid h-16 w-16 place-items-center rounded-full bg-black/40 backdrop-blur-md border border-white/10 shadow-lg scale-100 animate-in zoom-in-90">
        ...
        <div className="absolute bottom-0 left-0 right-0 z-30 p-4">
          <div className="flex items-center gap-3">
            <button onClick={onOpenProfile} className="h-12 w-12 overflow-hidden rounded-full border-2 border-white/20">
              <img src={item?.creator?.avatar_url || DEFAULT_AVATAR} alt="Creator" className="h-full w-full object-cover" />
            </button>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold">{item?.creator?.display_name || "Unknown"}</p>
              <p className="text-[11px] text-neutral-300">{formatCompact(item.views_count)} views</p>
            </div>
          </div>
          {item.caption && (
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          <div className="grid gap-6 md:grid-cols-[240px_1fr]">
            ...
            <div className="space-y-5">
              <button onClick={() => inputRef.current?.click()} disabled={uploading} className="w-full rounded-xl border border-white/5 bg-white/5 p-4 text-left">
                <p className="text-sm font-semibold">{file ? file.name : "Choose video file"}</p>
              </button>
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={onClose} disabled={uploading} className="flex-1 py-3 rounded-full border border-white/10 text-sm">Cancel</button>
                <button onClick={onUpload} disabled={uploading || !file} className="flex-1 py-3 rounded-full bg-white text-black text-sm font-semibold">Publish</button>
              </div>
            </div>
          </div>
        </div>
...
export default function Streams() {
  ...
  const [muted, setMuted] = useState(() => localStorage.getItem("streams_muted") !== "false");
  ...
  return (
    <div className="h-dvh bg-neutral-950">
      <div className="fixed top-4 right-4 z-50">
        <button onClick={() => setUploadOpen(true)} className="grid h-10 w-10 place-items-center rounded-full bg-white text-black shadow-lg hover:scale-105 active:scale-95 transition-all">
