...
export default function Discover() {
  ...
  const myLoc = useMemo(() => {
    ...
    if (profile?.lat && profile?.lng) return { lat: +profile.lat, lng: +profile.lng, isRealtime: false };
    return null;
  }, [geo, profile?.lat, profile?.lng]);

  const loadProfiles = useCallback(async (targetMode, silent = false) => {
    ...
    try {
      ...
      if (id === reqId.current && err.name !== "AbortError") setError(err.message);
    } finally {
      if (id === reqId.current && !silent) setLoading(false);
    }
  }, [userId, myLoc?.lat, myLoc?.lng]);

  // Initial Load & Cache Logic
  useEffect(() => {
    ...
    if (cached.items?.length) {
      ...
      setActiveMode(mode);
      setLoading(false);
      if (DiscoverCache.isStale(cached.ts)) loadProfiles(mode, true);
    } else {
      loadProfiles(mode);
    }
  }, [mode, userId, loadProfiles]);
  ...
  useEffect(() => {
    ...
    ]).then(([users, mine]) => setStories({ users, mine }));
  }, []);

  useRevalidate({
    refetch: () => loadProfiles(mode, true),
    intervalMs: 60000,
  });
    ...
    <div className="flex min-h-screen flex-col bg-white text-gray-900">
      <header className="px-4 pt-4">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex gap-2">
            <IconButton icon="bell" count={3} onClick={() => navigate("/notifications")} />
            <IconButton icon="adjustments" onClick={() => navigate("/filters")} />
          </div>
          ...
          <Link to="/profile" className="relative group">
            <img src={profile?.avatar_url || "/me.jpg"} className="h-10 w-10 rounded-full object-cover ring-2 ring-violet-600 ring-offset-2 transition-transform group-active:scale-90" />
          </Link>
        </div>
        ...
        <h1 className="text-xl font-bold mt-2">Find Your <span className="text-violet-600">Matches</span></h1>

        <div className="mt-3 flex gap-1 rounded-full bg-gray-100 p-1 w-fit">
          {TABS.map((t) => (
            <button
              key={t.key}
              onClick={() => { setMode(t.key); setProfiles([]); }}
              className={`rounded-full px-5 py-2 text-sm font-semibold transition-all ${
                mode === t.key ? "bg-white text-violet-600 shadow-sm" : "text-gray-500 hover:text-gray-700"
              }`}
            >
              {t.label}
            </button>
      ...
      <main className="flex-1 px-4 py-6">
        {loading && !profiles.length ? (
          ...
          <ErrorState error={error} onRetry={() => loadProfiles(mode)} />
        ) : (
          <div className="relative h-full">
            {mode === "nearby" && !myLoc && (
              <LocationPrompt onEnable={geoRefresh} loading={geoStatus === "loading"} />
            )}
            <SwipeDeck initialItems={profiles} mode={activeMode} myLoc={myLoc} />
          </div>
        )}
      </main>
    </div>
...
const IconButton = memo(({ icon, onClick, count }) => (
  <button onClick={onClick} className="relative rounded-full bg-gray-50 p-2.5 text-gray-700 hover:bg-gray-100 transition-colors">
    {icon === "bell" ? (
      <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V4a2 2 0 10-4 0v1.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0a3 3 0 11-6 0m6 0H9" /></svg>
    ) : (
    ...
    {count > 0 && <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white ring-2 ring-white">{count}</span>}
  </button>
));
...
const StoriesRow = memo(({ data, onMine, onUser }) => (
  <div className="no-scrollbar flex gap-4 overflow-x-auto py-2">
    ...
    {data.users.map(u => (
      <button key={u.user_id} onClick={() => onUser(u.user_id)} className="flex flex-col items-center gap-1 shrink-0">
        <img src={u.avatar || "/me.jpg"} className="h-14 w-14 rounded-full border-2 border-violet-200 p-0.5 object-cover" />
        <span className="text-[10px] font-medium text-gray-600 w-14 truncate text-center">{u.name?.split(' ')[0]}</span>
      </button>
    ))}
  </div>
));
...
const ErrorState = ({ error, onRetry }) => (
  <div className="flex h-64 flex-col items-center justify-center p-6 text-center">
    ...
    <button onClick={onRetry} className="mt-4 text-xs font-bold text-violet-600 uppercase tracking-widest">Try Again</button>
  </div>
);
...
const LocationPrompt = ({ onEnable, loading }) => (
  <div className="mb-6 flex items-center justify-between rounded-2xl bg-violet-600 p-4 text-white shadow-lg shadow-violet-200">
    ...
    <button onClick={onEnable} disabled={loading} className="rounded-full bg-white px-4 py-2 text-xs font-bold text-violet-600 active:scale-95 transition-transform">
