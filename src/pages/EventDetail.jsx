...
/* ================================================================
   MAP — lazy loaded to avoid Leaflet crashing at module level
   ================================================================ */
        ...
        if (title) {
          marker.bindPopup(
            `<div style="font-size:13px;font-weight:600;color:#1f2937;min-width:120px">
              ${title}
              ${address ? `<p style="font-size:11px;color:#6b7280;margin-top:2px;font-weight:400">${address}</p>` : ""}
            </div>`
          );
        }
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-gray-500 leading-relaxed max-w-xs mx-auto">
                {isFree
                  ...
                  : `Your ${qty} ticket${
                      qty > 1 ? "s" : ""
                    } are booked. Check your email for confirmation.`}
              </p>
            </div>
            ...
            {soldOut ? (
              ...
              <>
                ...
                >
                  {isFree
                    ? "Reserve My Spot"
                    : `Continue · ${priceFmt.format(total)}`}
                </button>
              </>
            )}
          </div>
        ...
        {step === "confirm" && (
          <div className="px-6 py-5 space-y-5">
            <div className="rounded-2xl bg-violet-50 border border-violet-100 p-4 space-y-2">
              ...
              {!isFree && (
                <>
                  <Row label="Subtotal" value={priceFmt.format(subtotal)} />
                  <Row label="Fees" value={priceFmt.format(fees)} />
                  <div className="flex justify-between text-sm border-t border-violet-100 pt-2">
                    <span className="font-bold text-gray-900">Total</span>
                    <span className="font-bold text-violet-700 text-base">
                      {priceFmt.format(total)}
            ...
            {!isFree && (
              <div className="space-y-2">
                ...
                <div className="rounded-2xl border-2 border-dashed border-gray-200 p-6 flex flex-col items-center gap-2 text-gray-400">
...
/* small reusable row for modal summaries */
...
export default function EventDetail() {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between gap-4 bg-white rounded-3xl p-4 border border-gray-100 shadow-sm">
            <div className="flex items-center gap-3 min-w-0">
              <div className="relative shrink-0">
                <img
                  src={
                    event.host.avatar || avatarFallback(event.host.name)
                  }
                  onError={(e) => {
      ...
      {/* Tickets Modal — rendered outside page scroll */}
      {showTickets && (
...
/* ================================================================
   ICONS — all self-contained SVGs, nothing imported from a library
   ================================================================ */
