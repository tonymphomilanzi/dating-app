  ...
  return {
    ...
    host: {
      name:
        ...
        "Event Organiser",
      avatar:
        raw.host?.avatar || raw.organizer?.avatar || raw.organiser?.avatar || "",
...
function formatDateTime(isoString) {
  ...
  if (isNaN(d)) return { date: "Date TBA", time: "", relative: "", iso: "" };
  return {
...
function GetTicketsModal({ event, onClose }) {
  ...
  const soldOut = spotsLeft === 0 && event.capacity != null;
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-lg font-extrabold text-gray-900">
              {step === "success" ? "You're going! 🎉" : isFree ? "RSVP" : "Get Tickets"}
            </h2>
          ...
          <div className="px-6 py-10 flex flex-col items-center text-center gap-5">
            ...
            <div>
              <h3 className="text-xl font-bold text-gray-900">
                {isFree ? "RSVP Confirmed!" : "Booking Confirmed!"}
              </h3>
              <p className="mt-2 text-sm text-gray-500 leading-relaxed">
                {isFree
                  ? "Your spot has been reserved. We'll send you a reminder before the event."
                  : `Your ${qty} ticket${qty > 1 ? "s" : ""} have been booked. Check your email for confirmation.`}
              </p>
            </div>
            <div className="w-full bg-violet-50 rounded-2xl p-4 text-left space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-gray-900">
                  {formatDateTime(event.starts_at).date}
                </span>
              </div>
              {!isFree && (
                <div className="flex justify-between text-sm">
              ...
              {!isFree && (
                <div className="flex justify-between text-sm border-t border-violet-100 pt-2 mt-2">
            ...
            {!soldOut && (
              <>
                  ...
                  {ticketTypes.map((tt) => (
                    ...
                    >
                      ...
                      <span className="font-bold text-sm text-violet-700">
                        {tt.price === 0 ? "Free" : priceFmt.format(tt.price)}
                      </span>
                    </button>
                  ))}
                </div>
                ...
                <button
                  ...
                  className="w-full h-14 rounded-2xl bg-gradient-to-r from-violet-600 to-violet-500 text-white font-extrabold text-base shadow-lg shadow-violet-600/30 hover:from-violet-500 hover:to-violet-400 active:scale-95 transition-all duration-150"
                >
                  {isFree ? "Reserve My Spot" : `Continue · ${priceFmt.format(total)}`}
                </button>
              </>
            )}
          </div>
        ...
        {step === "confirm" && (
          <div className="px-6 py-5 space-y-5">
            <div className="rounded-2xl bg-violet-50 border border-violet-100 p-4 space-y-3">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-gray-900">
                    {isFree ? "Free Admission" : `${qty} × ${selectedTicket?.label}`}
                  </span>
                </div>
                {!isFree && (
                  <>
            ...
            {/* Payment placeholder for paid events */}
            {!isFree && (
              <div className="space-y-2.5">
                <p className="text-xs font-bold uppercase tracking-widest text-gray-400">
                  Payment
                </p>
                <div className="rounded-2xl border-2 border-dashed border-gray-200 p-5 flex flex-col items-center gap-2 text-gray-400">
                  <CreditCardIcon className="h-8 w-8 opacity-40" />
                  <p className="text-sm text-center">
                    Payment integration goes here
                    <br />
                    <span className="text-xs">(Stripe / Apple Pay / etc.)</span>
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={loading}
                className="flex-1 h-14 rounded-2xl border-2 border-gray-200 bg-white text-gray-700 font-bold text-sm hover:bg-gray-50 active:scale-95 transition-all duration-150 disabled:opacity-50"
              >
                Back
              </button>
              <button
                ...
                disabled={loading}
                className="flex-[2] h-14 rounded-2xl bg-gradient-to-r from-violet-600 to-violet-500 text-white font-extrabold text-base shadow-lg shadow-violet-600/30 hover:from-violet-500 hover:to-violet-400 active:scale-95 transition-all duration-150 disabled:opacity-70 flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
            ...
            <p className="text-[10px] text-gray-400 text-center leading-relaxed">
              ...
              {!isFree && " Tickets are non-refundable."}
            </p>
          </div>
        )}
      </div>
...
export default function EventDetail() {
  ...
  /* ──────────────────────────────────────────────────────────────
     FETCH — simple boolean flag, no AbortController, no closures
     that could interfere with navigation.
  ────────────────────────────────────────────────────────────── */
  useEffect(() => {
    let cancelled = false;
...
function ExpandableText({ text, maxChars = 200 }) {
  ...
  const displayed =
    expanded || !needsTruncation ? text : `${text.slice(0, maxChars).trimEnd()}…`;
