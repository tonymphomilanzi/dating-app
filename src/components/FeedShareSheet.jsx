...
function ShareTile({ icon, label, onClick, href }) {
  const inner = (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <div className="grid h-12 w-12 place-items-center rounded-full bg-white border border-gray-100 shadow-sm">
        {icon}
      </div>
...
export default function FeedShareSheet({ feed, onClose, onShare }) {
  ...
  /*
    THE KEY URL:
    Must match exactly what api/me.js handleShare checks:
      req.method === "GET" && q1(req.query?.share) === "feed"
      req.query.id = feed.id
  */
  const ogUrl = useMemo(
    () => `${window.location.origin}/api/me?share=feed&id=${encodeURIComponent(feed.id)}`,
    [feed.id]
  );

  const spaUrl = useMemo(
  ...
  const logShare = useCallback(() => { onShare?.(); }, [onShare]);
  ...
  const systemShare = useCallback(async () => {
    ...
    await copyLink();
  }, [title, excerpt, ogUrl, onShare, onClose, copyLink]);
  ...
  return (
    <>
      ...
      <div
        ...
        className="fixed inset-x-0 bottom-0 z-50 flex flex-col bg-white rounded-t-3xl shadow-2xl max-h-[92vh]"
      >
        {/* Drag handle */}
        ...
        <div className="flex items-center justify-between px-5 pt-2 pb-3 shrink-0 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5 truncate max-w-[260px]">{title}</p>
          </div>
          <button
            onClick={onClose}
            aria-label="Close"
        ...
        {/* Scrollable body */}
        <div
          className="flex-1 overflow-y-auto px-4 pt-4 space-y-4 min-h-0"
          style={{ paddingBottom: "max(env(safe-area-inset-bottom), 24px)" }}
        >
          {/* Post preview card — mirrors what WhatsApp will render */}
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3">
            ...
            <button
              onClick={copyLink}
              className={[
                "flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-bold transition-all active:scale-95 shrink-0",
                copied
          ...
          <div className="grid grid-cols-4 gap-2.5 sm:grid-cols-5">
            <ShareTile icon={<Share2 className="h-5 w-5 text-gray-600" />}           label="More"        onClick={systemShare} />
            <ShareTile icon={<XTwitterIcon className="h-5 w-5 text-gray-900" />}     label="X / Twitter" href={hrefX}   onClick={logShare} />
            <ShareTile icon={<WhatsAppIcon className="h-5 w-5 text-emerald-500" />}  label="WhatsApp"    href={hrefWa}  onClick={logShare} />
            <ShareTile icon={<TelegramIcon className="h-5 w-5 text-sky-500" />}      label="Telegram"    href={hrefTg}  onClick={logShare} />
            <ShareTile icon={<FacebookIcon className="h-5 w-5 text-blue-600" />}     label="Facebook"    href={hrefFb}  onClick={logShare} />
            <ShareTile icon={<RedditIcon className="h-5 w-5 text-orange-500" />}     label="Reddit"      href={hrefRed} onClick={logShare} />
            <ShareTile icon={<LinkedInIcon className="h-5 w-5 text-sky-700" />}      label="LinkedIn"    href={hrefLi}  onClick={logShare} />
            <ShareTile icon={<Mail className="h-5 w-5 text-gray-600" />}             label="Email"       href={hrefMail} onClick={logShare} />
          </div>

          <p className="text-center text-[10px] text-gray-400 pb-2">
            Link previews may take a moment to appear
          </p>
        </div>
      </div>
    </>
