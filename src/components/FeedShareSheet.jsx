...
function ShareTile({ icon, label, onClick, href, colorClass = "" }) {
  const inner = (
    ...
    >
      <div
        className={`grid h-12 w-12 place-items-center rounded-full bg-white border border-gray-100 shadow-sm ${colorClass}`}
      >
        {icon}
      </div>
...
/* ================================================================
   ...
   Props:
     ...
     onShare  — optional callback to log the share to DB
   ================================================================ */
export default function FeedShareSheet({ feed, onClose, onShare }) {
  ...
  /* ── URLs ──
     ogUrl  → points to the API endpoint that serves OG HTML to bots
              and instantly redirects humans to the SPA.
              This is what you share everywhere.
     spaUrl → the clean SPA route, shown in the copy-bar for display only.
  ── */
  const ogUrl = useMemo(
    () =>
  ...
  return (
    <>
      ...
      <div
        ...
        className="fixed inset-x-0 bottom-0 z-50 flex flex-col bg-white rounded-t-3xl shadow-2xl max-h-[92vh] animate-in slide-in-from-bottom duration-300"
      >
        {/* Drag handle */}
        ...
        >
          ...
          {/* ── Post preview card ──
              Mirrors what WhatsApp / Telegram will render
              when the ogUrl link is unfurled.
          ── */}
          <div className="rounded-2xl border border-gray-100 overflow-hidden shadow-sm bg-white">
            ...
            {feed.image_url && !imgError && (
              ...
              >
                ...
                {/* Domain strip (mimics iMessage OG cards) */}
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3">
            ...
            {/* Show the clean SPA URL for aesthetics */}
          ...
          <p className="text-center text-[10px] text-gray-400 pb-2">
            Sharing a link to this post · previews may take a moment to appear
          </p>
        </div>
      </div>
    </>
