...
function ShareTile({ icon, label, onClick, href, colorClass = "" }) {
  const inner = (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full flex-col items-center gap-2 rounded-2xl border border-gray-100 bg-gray-50 p-3 transition-all hover:bg-gray-100 active:scale-95"
    >
...
export default function FeedShareSheet({ feed, userId, onClose, onShare }) {
  ...
  const shareText = feed.content.length > 120
    ? feed.content.slice(0, 120) + "…"
    : feed.content;

  const encodedUrl   = encodeURIComponent(url);
  const encodedTitle = encodeURIComponent(title);
  const encodedFull  = encodeURIComponent(`${title}\n\n${url}`);

  const hrefX       = `https://twitter.com/intent/tweet?text=${encodeURIComponent(`${title}\n\n`)}&url=${encodedUrl}`;
  ...
  const hrefMail    = `mailto:?subject=${encodedTitle}&body=${encodedFull}`;
  ...
  return (
    <>
      ...
      {/* Sheet */}
      <div className="fixed inset-x-0 bottom-0 z-50 flex flex-col bg-white rounded-t-3xl shadow-2xl max-h-[90vh] overflow-hidden">
        ...
        >
          ...
          {/* ── Post preview card (like YouTube/Instagram) ── */}
          <div className="rounded-2xl border border-gray-100 bg-gray-50 overflow-hidden shadow-sm">
            {/* Cover image */}
            {feed.image_url && !imgError && (
              <div className="relative w-full bg-gray-200 overflow-hidden" style={{ maxHeight: 200 }}>
                <img
                  src={feed.image_url}
                  alt={title}
                ...
                {feed.pinned && (
                  <div className="absolute top-2 left-2 flex items-center gap-1 rounded-full bg-violet-600 px-2.5 py-1">
            ...
            <div className="p-3 space-y-2">
              ...
              <div className="flex items-center gap-2">
                {feed.admin?.avatar_url ? (
                  <img
                    ...
                    alt={adminName}
                    className="h-7 w-7 rounded-full object-cover ring-1 ring-violet-100"
                  />
                ) : (
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-bold text-gray-800">{adminName}</span>
              ...
              {feed.tags?.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {feed.tags.slice(0, 4).map((tag) => (
                    <span key={tag} className="rounded-full bg-gray-200 px-2 py-0.5 text-[10px] font-semibold text-gray-500">
                      #{tag}
                    </span>
                  ))}
                </div>
              ...
              {/* URL pill */}
              <div className="flex items-center gap-1.5 rounded-lg bg-white border border-gray-100 px-2.5 py-1.5">
